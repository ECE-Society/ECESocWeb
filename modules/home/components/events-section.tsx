                        ...
                        {events.map((event, index) => {
                            ...
                            return (
                                <SectionReveal
                                    key={index}
                                    delay={index * 0.1}
                                    className={`w-full ${widths[index % 4]} p-10 md:p-12 bg-[#111]/80 backdrop-blur-md rounded-[3rem] border border-white/5 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.8)] relative group hover:border-[#2DD4BF]/40 transition-all duration-500 overflow-hidden cursor-pointer`}
                                >
                                    <Link href="/events" className="absolute inset-0 z-20" />
                                    {/* Animated background glow */}
                                    <div className="absolute -inset-1 bg-gradient-to-br from-[#2DD4BF]/20 via-transparent to-purple-500/10 transition-opacity duration-500 blur-2xl pointer-events-none opacity-0 group-hover:opacity-100" />
                                    <div className="absolute inset-0 bg-gradient-to-br from-[#2DD4BF]/5 to-transparent transition-opacity rounded-[3rem] pointer-events-none opacity-0 group-hover:opacity-100" />
